...
**Progress:** ✅ Phase 0 complete · ✅ Phase 1 complete. Notes: Next 16 renamed
middleware to `proxy.ts` (used); Geist fonts self-hosted (no Google Fonts at build
time); shadcn/ui components vendored by hand because the registry is unreachable
from the build sandbox (`components.json` kept so `shadcn add` works elsewhere).
